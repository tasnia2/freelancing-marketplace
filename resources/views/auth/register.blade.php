...
@section('content')
<div x-data="registerWizard()" x-init="init()" x-cloak>
            ...
            @foreach(['Account', 'Profile', 'Role'] as $index => $title)
                <div class="text-center">
                    ...
                    <span class="text-xs font-medium"
                          :class="step >= {{ $index + 1 }} ? 'text-white/85' : 'text-white/55'">
                        {{ $title }}
                    </span>
                </div>
            @endforeach
    ...
    <form method="POST" action="{{ route('register') }}" id="registerForm">
        ...
        <div x-show="step === 1" x-transition.opacity>
            <h3 class="text-lg font-bold text-white mb-6">Create Your Account</h3>

            <!-- Name -->
            <div class="mb-5">
                <label class="block text-sm font-medium text-white/75 mb-2">Full Name *</label>
                <input type="text" x-model="form.name" required
                       class="w-full input-field"
                       placeholder="John Doe">
                <div x-show="errors.name" class="mt-1 text-sm text-red-300" x-text="errors.name"></div>
                @error('name')
            ...
            <!-- Email -->
            <div class="mb-5">
                <label class="block text-sm font-medium text-white/75 mb-2">Email Address *</label>
                <input type="email" x-model="form.email" required
                       class="w-full input-field"
                       placeholder="you@example.com">
                <div x-show="errors.email" class="mt-1 text-sm text-red-300" x-text="errors.email"></div>
                @error('email')
                    <p class="mt-1 text-sm text-red-300">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-8">
                ...
                <div x-show="form.password.length > 0" class="mt-3">
                    <div class="flex items-center">
                        <span class="text-sm font-medium text-white/70 mr-2">Strength:</span>
                        <span class="text-sm font-semibold"
                              :class="{
                                  'text-red-300': passwordStrength.score <= 1,
                                  'text-yellow-300': passwordStrength.score == 2,
                                  'text-sky-300': passwordStrength.score == 3,
                                  'text-green-300': passwordStrength.score >= 4
                              }"
                              x-text="passwordStrength.text"></span>
                    </div>
                </div>

                <p class="mt-3 text-xs text-white/55">Minimum 8 characters</p>

                <div x-show="errors.password" class="mt-1 text-sm text-red-300" x-text="errors.password"></div>
                @error('password')
                    <p class="mt-1 text-sm text-red-300">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div x-show="step === 2" x-transition.opacity>
            <h3 class="text-lg font-bold text-white mb-6">Complete Your Profile</h3>

            <!-- Profile Picture -->
            <div class="mb-6">
                <label class="block text-sm font-medium text-white/75 mb-3">Profile Picture (Optional)</label>

                <div class="flex items-center space-x-4">
                    <!-- Preview Container -->
                    <div class="relative">
                        <div class="w-20 h-20 rounded-full border-2 border-white/20 overflow-hidden bg-white/10 flex items-center justify-center">
                            <!-- Image Preview -->
                        ...
                        <label for="profile-picture"
                               class="absolute -bottom-1 -right-1 bg-white/20 text-white p-1.5 rounded-full cursor-pointer shadow-sm hover:bg-white/30 transition text-xs">
                            <i class="fas fa-camera"></i>
                        </label>

                        <!-- Delete Button (if image exists) -->
                        <button type="button" x-show="form.profilePicture" @click="form.profilePicture = ''"
                                class="absolute -bottom-1 -left-1 bg-red-500/80 text-white p-1.5 rounded-full cursor-pointer shadow-sm hover:bg-red-600 transition text-xs">
                    ...
                    <div class="flex-1">
                        <div class="space-y-2">
                            ...
                            <!-- Upload Button -->
                            <label for="profile-picture"
                                   class="inline-flex items-center px-4 py-2 bg-white/10 hover:bg-white/15 text-white/85 text-sm rounded-lg cursor-pointer transition border border-white/15">
                                <i class="fas fa-upload mr-2 text-sm"></i> Choose Photo
                            </label>

                            <!-- Current File Info -->
                            <div x-show="form.profilePicture" class="text-xs text-green-300">
            ...
            <!-- Phone -->
            <div class="mb-5">
                <label class="block text-sm font-medium text-white/75 mb-2">Phone Number</label>
            ...
            <!-- Location -->
            <div class="mb-5">
                <label class="block text-sm font-medium text-white/75 mb-2">Location</label>
            ...
            <!-- Bio -->
            <div class="mb-8">
                <label class="block text-sm font-medium text-white/75 mb-2">Bio (Optional)</label>
        ...
        <div x-show="step === 3" x-transition.opacity>
            <h3 class="text-lg font-bold text-white mb-6">Choose Your Role</h3>

            <!-- Role Cards -->
            <div class="space-y-4 mb-8">
                <!-- Freelancer -->
                <div @click="form.role = 'freelancer'"
                     :class="form.role === 'freelancer' ? 'active ring-2 ring-white/25' : ''"
                     class="role-card bg-white/10 border border-white/15 text-white">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                            <div class="w-12 h-12 rounded-lg bg-white/15 flex items-center justify-center">
                                <i class="fas fa-laptop-code text-white text-lg"></i>
                            </div>
                        </div>
                        <div class="ml-4 flex-1">
                            <h4 class="font-bold text-white mb-1">I'm a Freelancer</h4>
                            <p class="text-white/65 text-xs mb-2">
                                I want to offer my skills and find projects
                            </p>
                            <div class="space-y-1 text-xs text-white/75">
                                <div class="flex items-center">
                        ...
                        <div class="flex-shrink-0 ml-4">
                            <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center"
                                 :class="form.role === 'freelancer'
                                     ...
                                     : 'border-white/25'">
                                <i class="fas fa-check text-[#1B3C53] text-xs"
                ...
                <div @click="form.role = 'client'"
                     :class="form.role === 'client' ? 'active ring-2 ring-white/25' : ''"
                     class="role-card bg-white/10 border border-white/15 text-white">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                            <div class="w-12 h-12 rounded-lg bg-white/15 flex items-center justify-center">
                        ...
                        <div class="ml-4 flex-1">
                            <h4 class="font-bold text-white mb-1">I'm a Client</h4>
                            <p class="text-white/65 text-xs mb-2">
            ...
            <div class="mb-6">
                <div class="flex items-start">
                    <input id="terms" type="checkbox" required
                           class="w-4 h-4 text-white rounded border-white/30 focus:ring-white/30 mt-0.5">
                    <label for="terms" class="ml-2 text-sm text-white/70">
                        I agree to the <a href="#" class="text-white underline underline-offset-2">Terms</a>
                        and <a href="#" class="text-white underline underline-offset-2">Privacy Policy</a>
                    </label>
                </div>
            </div>
...
function registerWizard() {
    return {
        ...
        checkPasswordStrength() {
            ...
            if (score === 0) this.passwordStrength.text = 'Very Weak';
            else if (score === 1) this.passwordStrength.text = 'Weak';
            else if (score === 2) this.passwordStrength.text = 'Fair';
            else if (score === 3) this.passwordStrength.text = 'Good';
            else if (score === 4) this.passwordStrength.text = 'Strong';
            else this.passwordStrength.text = 'Very Strong';
        },
